...
if __name__ == "__main__":
    ...
    def run_one_split_with_param_set(save_path, split_info, param_kwargs):
        ...
        loader_kwargs = {
            k: template_loader_kwargs["train"]
            if "train" in k
            else template_loader_kwargs["infer"]
        ...
        model_config = cfg_getter(
            train_loader_list, infer_loader_list, **template_paramset
        )

        def create_dataset(run_mode=None, subset_name=None, setup_augmentor=None):
            target_gen_func = getattr(
                importlib.import_module("models.hovernet.targets"), "gen_targets"
            )
